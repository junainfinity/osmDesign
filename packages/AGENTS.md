...
- `packages/sidecar-proto`: osmDesign sidecar business protocol. Owns app/mode/source constants, namespace validation, stamp descriptor/fields/flags, IPC message schema, status shapes, error semantics, and default product path constants.
- `packages/sidecar`: generic sidecar runtime primitives. Includes bootstrap, IPC transport, path/runtime resolution, launch env, and JSON runtime file helpers; it must not hard-code osmDesign app keys or IPC business messages.
...
- Do not hard-code osmDesign app/source/mode constants in `sidecar` or `platform`.
- Keep stamp fields limited to five: `app`, `mode`, `namespace`, `ipc`, and `source`.

## Common package commands

```bash
pnpm --filter @osmdesign/contracts typecheck
pnpm --filter @osmdesign/sidecar-proto typecheck
pnpm --filter @osmdesign/sidecar-proto test
pnpm --filter @osmdesign/sidecar typecheck
pnpm --filter @osmdesign/sidecar test
pnpm --filter @osmdesign/platform typecheck
pnpm --filter @osmdesign/platform test
```
