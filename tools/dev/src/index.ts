...
import { createSidecarLaunchEnv, requestJsonIpc } from "@osmdesign/sidecar";
import {
...
async function buildDesktop(config: ToolDevConfig, logHandle: FileHandle): Promise<void> {
  await logHandle.write(`\n[tools-dev] building @osmdesign/desktop at ${new Date().toISOString()}\n`);
  const invocation = createPackageManagerInvocation(["--filter", "@osmdesign/desktop", "build"], process.env);
