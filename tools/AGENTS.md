...
- `tools/dev` provides `@osmdesign/tools-dev` and the `tools-dev` bin. It is the only currently active local development lifecycle control plane.
...
- `tools/pack` provides `@osmdesign/tools-pack` and the `tools-pack` bin. The active slice is mac-first packaged artifact build/install/start/stop/logs/uninstall/cleanup plus beta release artifact preparation.
...
- Orchestration layers must consume primitives from `@osmdesign/sidecar-proto`, `@osmdesign/sidecar`, and `@osmdesign/platform`.
...
```bash
pnpm --filter @osmdesign/tools-dev typecheck
pnpm --filter @osmdesign/tools-dev build
pnpm --filter @osmdesign/tools-pack typecheck
