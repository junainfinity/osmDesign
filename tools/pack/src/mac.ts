...
import { createSidecarLaunchEnv, requestJsonIpc, resolveAppIpcPath } from "@osmdesign/sidecar";
import {
  ...
  stopProcesses,
} from "@osmdesign/platform";

import type { ToolPackBuildOutput, ToolPackConfig } from "./config.js";
import { macResources } from "./resources.js";

const execFileAsync = promisify(execFile);
const PRODUCT_NAME = "osmDesign";

const INTERNAL_PACKAGES = [
  { directory: "packages/contracts", name: "@osmdesign/contracts" },
  { directory: "packages/sidecar-proto", name: "@osmdesign/sidecar-proto" },
  { directory: "packages/sidecar", name: "@osmdesign/sidecar" },
  { directory: "packages/platform", name: "@osmdesign/platform" },
  { directory: "apps/daemon", name: "@osmdesign/daemon" },
  { directory: "apps/web", name: "@osmdesign/web" },
  { directory: "apps/desktop", name: "@osmdesign/desktop" },
  { directory: "apps/packaged", name: "@osmdesign/packaged" },
] as const;
...
function resolveMacPaths(config: ToolPackConfig): MacPaths {
  ...
  return {
    ...
    packagedConfigPath: join(namespaceRoot, "osmdesign-config.json"),
    resourceRoot: join(namespaceRoot, "resources", "osmdesign"),
...
async function buildWorkspaceArtifacts(config: ToolPackConfig): Promise<void> {
  ...
  await runPnpm(config, ["--filter", "@osmdesign/sidecar-proto", "build"]);
  await runPnpm(config, ["--filter", "@osmdesign/sidecar", "build"]);
  await runPnpm(config, ["--filter", "@osmdesign/platform", "build"]);
  await runPnpm(config, ["--filter", "@osmdesign/daemon", "build"]);
  try {
    await runPnpm(config, ["--filter", "@osmdesign/web", "build"], {
      OD_WEB_OUTPUT_MODE: "server",
    });
    await runPnpm(config, ["--filter", "@osmdesign/web", "build:sidecar"]);
  } finally {
  ...
  await runPnpm(config, ["--filter", "@osmdesign/desktop", "build"]);
  await runPnpm(config, ["--filter", "@osmdesign/packaged", "build"]);
}
...
async function writeAssembledApp(
  ...
  await writeFile(
    ...
    `${JSON.stringify(
      {
        dependencies,
        description: "osmDesign packaged runtime",
        main: "./main.cjs",
        name: "osmdesign-packaged-app",
        private: true,
    ...
    `${JSON.stringify(
      {
        ...
        nodeCommandRelative: "osmdesign/bin/node",
...
async function runElectronBuilder(
  ...
  const builderConfig = {
    ...
    extraResources: [
      { from: paths.resourceRoot, to: "osmdesign" },
      { from: paths.packagedConfigPath, to: "osmdesign-config.json" },
    ],
    ...
    publish: [
      {
        provider: "generic",
        url: "https://updates.invalid/osmdesign",
      },
    ],
  };
...
async function resolveDesktopRootIdentityFallback(config: ToolPackConfig): Promise<{
  ...
  try {
    stamp = OSM_DESIGN_SIDECAR_CONTRACT.normalizeStamp(marker.stamp);
  } catch {
    return {
...
export async function startPackedMacApp(config: ToolPackConfig): Promise<MacStartResult> {
  ...
  const spawned = await spawnBackgroundProcess({
    args: createProcessStampArgs(stamp, OSM_DESIGN_SIDECAR_CONTRACT),
