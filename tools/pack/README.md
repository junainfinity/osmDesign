...
Local packaging control plane for osmDesign.
...
Release artifacts keep the canonical `osmDesign.app` bundle shape; local `tools-pack install` copies it as
`osmDesign.<namespace>.app` so developer namespaces can coexist without affecting runtime data/log/cache paths.
