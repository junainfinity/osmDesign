...
test('parsePiModels parses TSV table with default option prepended', () => {
  const input =
    ...
    'osmapi           gpt-5.2              128K      16K      yes        yes\n' +
  ...
  assert.equal(result[1].id, 'osmapi/gpt-5.2');
...
test('parsePiModels deduplicates identical provider/model pairs', () => {
  const input =
    'provider         model                  context  max-out  thinking  images\n' +
    'openrouter       qwen3-coder             256K      16K      yes        no\n' +
    'openrouter       qwen3-coder             256K      16K      yes        no\n';
  ...
  assert.equal(result[1].id, 'openrouter/qwen3-coder');
});
...
test('parsePiModels skips duplicate default id', () => {
  ...
  assert.ok(result);
  assert.equal(result.length, 3); // synthetic default + default/some-model + osmapi/gpt-5.2
  assert.equal(result[0].id, 'default');
