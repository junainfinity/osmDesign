...
import { importDesignZip } from './design-zip-import.js';
...
/** @typedef {import('@osmdesign/contracts').ApiErrorCode} ApiErrorCode */
/** @typedef {import('@osmdesign/contracts').ApiError} ApiError */
/** @typedef {import('@osmdesign/contracts').ApiErrorResponse} ApiErrorResponse */
/** @typedef {import('@osmdesign/contracts').ChatRequest} ChatRequest */
/** @typedef {import('@osmdesign/contracts').ChatSseEvent} ChatSseEvent */
/** @typedef {import('@osmdesign/contracts').ProxyStreamRequest} ProxyStreamRequest */
/** @typedef {import('@osmdesign/contracts').ProxySseEvent} ProxySseEvent */
...
const OSM_API_HOST_SUFFIX = 'osmapi.com';
...
function isOsmApiHost(hostname) {
  const host = normalizeProxyHostname(hostname);
  return host === OSM_API_HOST_SUFFIX || host.endsWith(`.${OSM_API_HOST_SUFFIX}`);
}

export function classifyProxyBaseUrl(baseUrl) {
  let parsed;
  try {
    parsed = new URL(String(baseUrl || '').replace(/\/+$/, ''));
  } catch {
    return { allowed: false, reason: 'Invalid baseUrl' };
  }
  if (!['http:', 'https:'].includes(parsed.protocol)) {
    return { allowed: false, reason: 'Only http/https allowed' };
  }
  if (isOsmApiHost(parsed.hostname)) {
    if (parsed.protocol !== 'https:') {
      return { allowed: false, reason: 'osmAPI.com requires https' };
    }
    return {
      allowed: true,
      kind: 'osm',
...
export async function startServer({ port = 7456, returnServer = false } = {}) {
  ...
  app.get('/api/projects', (_req, res) => {
    try {
      ...
      /** @type {import('@osmdesign/contracts').ProjectsResponse} */
      const body = {
  ...
  app.post('/api/projects', async (req, res) => {
    try {
      ...
      /** @type {import('@osmdesign/contracts').CreateProjectResponse} */
  ...
  app.post('/api/import/design-zip', importUpload.single('file'), async (req, res) => {
    try {
      if (!req.file) return res.status(400).json({ error: 'zip file required' });
      const originalName = req.file.originalname || 'design export.zip';
      ...
      const id = randomId();
      const now = Date.now();
      const baseName = originalName.replace(/\.zip$/i, '').trim() || 'design import';
      const imported = await importDesignZip(req.file.path, projectDir(PROJECTS_DIR, id));
      fs.promises.unlink(req.file.path).catch(() => {});
      ...
      const project = insertProject(db, {
        ...
        skillId: null,
        designSystemId: null,
        pendingPrompt: `Imported from design ZIP: ${originalName}. Continue editing ${imported.entryFile}.`,
        metadata: {
          kind: 'prototype',
          importedFrom: 'design-zip',
  ...
  app.get('/api/projects/:id', (req, res) => {
    ...
    /** @type {import('@osmdesign/contracts').ProjectResponse} */
    const body = { project };
    res.json(body);
  });
  ...
  app.delete('/api/projects/:id', async (req, res) => {
    try {
      ...
      /** @type {import('@osmdesign/contracts').OkResponse} */
      const body = { ok: true };
      res.json(body);
    } catch (err) {
      sendApiError(res, 400, 'BAD_REQUEST', String(err));
  ...
  app.get('/api/deploy/config', async (_req, res) => {
    try {
      /** @type {import('@osmdesign/contracts').DeployConfigResponse} */
  ...
  app.get('/api/projects/:id/deployments', (req, res) => {
    try {
      /** @type {import('@osmdesign/contracts').ProjectDeploymentsResponse} */
  ...
  app.post('/api/projects/:id/deploy', async (req, res) => {
    try {
      ...
      const now = Date.now();
      /** @type {import('@osmdesign/contracts').DeployProjectFileResponse} */
  ...
  app.post('/api/projects/:id/deployments/:deploymentId/check-link', async (req, res) => {
    try {
      ...
      const now = Date.now();
      /** @type {import('@osmdesign/contracts').CheckDeploymentLinkResponse} */
      const body = upsertDeployment(db, {
        ...existing,
  ...
  app.get('/api/projects/:id/files', async (req, res) => {
    try {
      ...
      /** @type {import('@osmdesign/contracts').ProjectFilesResponse} */
      const body = { files };
      res.json(body);
    } catch (err) {
      sendApiError(res, 400, 'BAD_REQUEST', String(err));
  ...
  app.delete('/api/projects/:id/raw/*', async (req, res) => {
    try {
      ...
      /** @type {import('@osmdesign/contracts').DeleteProjectFileResponse} */
      const body = { ok: true };
      res.json(body);
    } catch (err) {
      const status = err && err.code === 'ENOENT' ? 404 : 400;
    ...
    async (req, res) => {
      try {
        ...
        if (req.file) {
          ...
          fs.promises.unlink(req.file.path).catch(() => {});
          /** @type {import('@osmdesign/contracts').ProjectFileResponse} */
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        /** @type {import('@osmdesign/contracts').UploadProjectFilesResponse} */
  ...
  app.post('/api/runs', (req, res) => {
    ...
    /** @type {import('@osmdesign/contracts').ChatRunCreateResponse} */
  ...
  app.get('/api/runs', (req, res) => {
    ...
    /** @type {import('@osmdesign/contracts').ChatRunListResponse} */
  ...
  app.post('/api/runs/:id/cancel', (req, res) => {
    ...
    /** @type {import('@osmdesign/contracts').ChatRunCancelResponse} */
    const body = { ok: true };
    res.json(body);
  });
  ...
  // Browser → daemon → osmAPI or a local/private OpenAI-compatible endpoint.
  ...
  app.post('/api/proxy/stream', async (req, res) => {
    ...
    const policy = classifyProxyBaseUrl(baseUrl);
    if (!policy.allowed) {
      return sendApiError(res, 400, 'FORBIDDEN', policy.reason);
    }
    if (policy.requiresApiKey && !String(apiKey || '').trim()) {
      return sendApiError(res, 400, 'BAD_REQUEST', 'apiKey is required for osmAPI');
    }
    ...
    console.log(`[proxy] ${req.method} ${parsed.hostname} kind=${policy.kind} model=${model} miMo=${isMiMo}`);
