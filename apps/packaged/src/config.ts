...
import { SIDECAR_DEFAULTS, normalizeNamespace } from "@osmdesign/sidecar-proto";
...
function resolveDefaultConfigPath(): string {
  return join(process.resourcesPath, "osmdesign-config.json");
}
...
async function readRawPackagedConfig(): Promise<RawPackagedConfig> {
  ...
  return (
    ...
    (await readJsonIfExists(join(app.getAppPath(), "osmdesign-config.json"))) ??
    {}
  );
}
...
export async function readPackagedConfig(): Promise<PackagedConfig> {
  ...
  const resourceRoot = resolveOptionalPath(raw.resourceRoot) ?? join(process.resourcesPath, "osmdesign");
  const relativeNodeCommand =
    raw.nodeCommandRelative == null || raw.nodeCommandRelative.length === 0
      ? join("osmdesign", "bin", "node")
