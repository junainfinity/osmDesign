...
import { createProcessStampArgs, stopProcesses, waitForProcessExit } from "@osmdesign/platform";
...
async function spawnSidecarChild(options: {
  ...
  const child = spawn(
    command,
    [options.entryPath, ...createProcessStampArgs(stamp, OSM_DESIGN_SIDECAR_CONTRACT)],
    {
      cwd: process.cwd(),
      env: childEnv,
...
export async function startPackagedSidecars(
  ...
  try {
    const daemon = await spawnSidecarChild({
      app: APP_KEYS.DAEMON,
      entryPath: resolveSidecarEntry("@osmdesign/daemon", "sidecar"),
      env: {
    ...
    const web = await spawnSidecarChild({
      app: APP_KEYS.WEB,
      entryPath: resolveSidecarEntry("@osmdesign/web", "sidecar"),
      env: {
