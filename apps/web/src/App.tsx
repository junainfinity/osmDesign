...
export function App() {
  ...
  const handleConfigSave = useCallback((next: AppConfig) => {
    ...
    const withOnboarding: AppConfig = { ...next, mediaProviders: {}, onboardingCompleted: true };
  ...
  const handleImportDesignZip = useCallback(async (file: File) => {
    const result = await importDesignZip(file);
    if (!result) return;
    setProjects((curr) => [result.project, ...curr.filter((p) => p.id !== result.project.id)]);
    navigate({
      kind: 'project',
    ...
    <>
      {activeProject ? (
        ...
        <EntryView
          ...
          onImportDesignZip={handleImportDesignZip}
