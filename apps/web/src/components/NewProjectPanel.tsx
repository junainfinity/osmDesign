...
export type CreateTab = 'prototype' | 'deck' | 'template';
...
export function NewProjectPanel({
  ...
  async function handleImportPicked(ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    ev.target.value = '';
    if (!file || !onImportDesignZip) return;
    setImporting(true);
    try {
      await onImportDesignZip(file);
    } finally {
      setImporting(false);
    }
  }
        ...
        {onImportDesignZip ? (
          <>
            ...
            <button
              ...
              title={t('newproj.importDesignZipTitle')}
              onClick={() => importInputRef.current?.click()}
            >
              <Icon name="import" size={13} />
              <span>
                {importing
                  ? t('newproj.importingDesignZip')
                  : t('newproj.importDesignZip')}
              </span>
            </button>
          </>
        ) : null}
...
/* ============================================================
   ...
   - Trigger card uses a compact swatch strip + title + "Default" subtitle
     + chevron so it stays scannable inside the create panel.
   ============================================================ */
...
function buildMetadata(input: {
  ...
  if (input.tab === 'prototype') {
    return { kind: 'prototype', fidelity: input.fidelity, ...inspirations };
  }
  if (input.tab === 'deck') {
    return { kind: 'deck', speakerNotes: input.speakerNotes, ...inspirations };
  }
  if (input.templateId == null) {
    return { kind: 'template', animations: input.animations, ...inspirations };
  }
