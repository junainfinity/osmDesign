...
import { streamMessage } from '../providers/ai-stream';
import {
  fetchChatRunStatus,
  listActiveChatRuns,
  reattachDaemonRun,
...
import { composeSystemPrompt } from '@osmdesign/contracts';
    ...
    async (prompt: string, attachments: ChatAttachment[]) => {
      ...
      const selectedAgent =
        config.mode === 'daemon' && config.agentId
          ...
          : null;
      const apiAgentId = config.apiProvider === 'local' ? 'local-ai-server' : 'osmapi';
      const apiAgentName = config.apiProvider === 'local' ? 'Local AI server' : 'osmAPI';
      const assistantAgentId =
        config.mode === 'daemon' ? config.agentId ?? undefined : apiAgentId;
