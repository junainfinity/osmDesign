...
async function runDesignSystemSelectionFlow(
  ...
  await page.getByTestId('design-system-search').fill('OSM');
  await page.getByRole('option', { name: /OSM Soft Tech/i }).click();
  await expect(page.getByTestId('design-system-trigger')).toContainText('OSM Soft Tech');
  await page.getByTestId('create-project').click();

  await expect(page).toHaveURL(/\/projects\//);
  await expect(page.getByTestId('project-meta')).toContainText('OSM Soft Tech');
  await expect(page.getByTestId('chat-composer')).toBeVisible();
}
