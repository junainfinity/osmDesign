...
const BETA_TAG = "osmdesign-beta";
const stableVersionPattern = /^(\d+)\.(\d+)\.(\d+)$/;
const stableTagPattern = /^osmdesign-v(\d+\.\d+\.\d+)$/;
const betaVersionPattern = /^(\d+\.\d+\.\d+)-beta\.(\d+)$/;
const betaTagPattern = /^osmdesign-v(\d+\.\d+\.\d+)-beta\.(\d+)(?:\.unsigned)?$/;
...
const signed = process.env.OSM_DESIGN_RELEASE_SIGNED !== "false";
...
const versionTag = `osmdesign-v${betaVersion}${unsignedSuffix}`;
const branch = process.env.GITHUB_REF_NAME ?? "";
const commit = process.env.GITHUB_SHA ?? "";
const releaseName = `osmDesign Beta ${betaVersion}${signed ? "" : " (unsigned)"}`;
